...
Usage
-----
    from peak_options import GasBoiler, ElectricBoiler
 
    # From a preset (Ealing report Phase 1/2 figures)
    gas_boiler = GasBoiler.from_preset("ealing_phase1", weather_df=None)
...
class GasBoiler:
    ...
    gas_price_GBP_per_MWh   : gas commodity price (£/MWh, gross CV basis)
                               Default 45.0 — broadly representative of
                               2024/25 UK industrial/commercial gas prices;
                               VERIFY against current DESNZ data before
                               using for real financial decisions.
    ...
    def __init__(
        ...
        self.gas_price_GBP_per_MWh = float(gas_price_GBP_per_MWh)
    ...
    def _recompute_marginal_cost(self):
        ...
        fuel_cost = self.gas_price_GBP_per_MWh / self.efficiency_hourly
        carbon_cost = (
    ...
    def from_preset(cls, preset_key: str, **overrides) -> "GasBoiler":
        ...
        -------
            ...
            boiler = GasBoiler.from_preset("ealing_phase1", gas_price_GBP_per_MWh=50.0)
        """
    ...
    def summary(self) -> dict:
        return {
            ...
            "gas_price_GBP_per_MWh":      self.gas_price_GBP_per_MWh,
...
class ElectricBoiler:
    ...
    electricity_price_GBP_per_MWh : scalar or 8760-array. Default 120.0 is
                                     a placeholder — verify against actual
                                     tariff data for real financial decisions.
...
if __name__ == "__main__":
    print("\n" + "="*70)
    print("  peak_options.py — self-test")
    print("="*70)
    ...
    # Test all gas boiler presets
    print("\n  All gas boiler presets (at full load):")
    print(f"  {'Preset':<25} {'Capacity MW':>12} {'Cost £/MWh':>12} {'Condensing':>11}")
    print("  " + "-"*65)
    for key in GAS_BOILER_PRESETS:
        b = GasBoiler.from_preset(key)
        s = b.summary()
        print(f"  {key:<25} {s['capacity_MW']:>12.1f} {s['mean_marginal_cost_GBP_per_MWh']:>12.2f} {str(s['condensing']):>11}")
 
    # Test all electric boiler presets
    print("\n  All electric boiler presets:")
    print(f"  {'Preset':<25} {'Capacity MW':>12} {'Cost £/MWh':>12} {'Efficiency':>11}")
    print("  " + "-"*65)
    for key in ELECTRIC_BOILER_PRESETS:
        b = ElectricBoiler.from_preset(key)
        s = b.summary()
        print(f"  {key:<25} {s['capacity_MW']:>12.1f} {s['mean_marginal_cost_GBP_per_MWh']:>12.2f} {s['efficiency']:>11.1%}")
 
    # Detailed test: Ealing Phase 1 gas boiler
    print("\n  Ealing Phase 1 gas boiler (detailed):")
    ...
    # Compare gas vs electric boiler cost at typical prices
    print("\n  Cost comparison — Gas vs Electric boiler (same 3.6 MW capacity):")
    gas = GasBoiler.from_preset("ealing_phase1")
    elec = ElectricBoiler.from_preset("ealing_backup")
    print(f"    Gas boiler:      £{gas.marginal_cost.mean():.2f}/MWh heat  (gas @ £45/MWh, η={gas.eta_full_load:.0%})")
    print(f"    Electric boiler: £{elec.marginal_cost.mean():.2f}/MWh heat (elec @ £120/MWh, η={elec.efficiency:.0%})")
    print(f"    → Electric is {elec.marginal_cost.mean()/gas.marginal_cost.mean():.1f}x more expensive per MWh heat at these prices")
