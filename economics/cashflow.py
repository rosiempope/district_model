...
def irr_from_cashflows(net_cashflow: Sequence[float], guess: float = 0.10) -> float | None:
    """Return the IRR root nearest a conventional project-finance guess.

    REPEX can create multiple sign changes and therefore multiple mathematical
    roots. The former implementation scanned from -95% upward and returned a
    large negative root even when the normal positive project IRR was present.
    We now find every bracketed root and select the one nearest 10%, matching
    the practical behaviour expected from spreadsheet IRR with a 10% guess.
    The cash-flow result still warns whenever roots may be non-unique.
    """
    values = np.asarray(net_cashflow, dtype=float)
    ...
    for root in roots:
        if not any(abs(root - existing) < 1e-6 for existing in unique):
            unique.append(root)
    return min(unique, key=lambda root: abs(root - float(guess)))
