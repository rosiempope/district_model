...
def _template_map() -> dict[str, dict[str, Any]]:
    technology_templates = [frontier_scenario(mix, 2.0, 1.0) for mix in MIX_NAMES]
    scenarios = [
    ...
    return {scenario["name"]: copy.deepcopy(scenario) for scenario in scenarios}
...
def _clear_editor_widget_state() -> None:
    """Clear every form widget when loading/rebuilding a scenario.

    A partial clear is unsafe: keyed CAPEX/OPEX widgets can otherwise survive
    a template switch or a hot reload and silently overwrite the newly loaded
    scenario. Only actual application state is retained.
    """
    retained = {"scenario", "comparison_results", "last_result"}
    for key in list(st.session_state.keys()):
        if key not in retained:
            del st.session_state[key]


def _is_ealing_validation_name(name: Any) -> bool:
    return str(name).strip() == EALING_PHASE1_VALIDATION["name"]


def _ealing_validation_rows(result: dict[str, Any]) -> list[dict[str, Any]]:
    """Return the small set of published calibration checks shown in the UI."""
    h = result["headline"]
    investor = result.get("financial", {}).get("investor", {})
    rows = [
        ("End-customer heat", h["annual_heat_demand_MWh"], REPORT_PHASE1_TARGETS["end_customer_heat_MWh"], "MWh/yr", 0.2),
        ("Peak heat", h["peak_heat_to_generate_MW"], REPORT_PHASE1_TARGETS["peak_heat_MW"], "MW", 0.002),
        ("Unmet heat", h["annual_unmet_demand_MWh"], 0.0, "MWh/yr", 0.05),
        ("Total CAPEX", h["capex_total_GBP"], REPORT_PHASE1_TARGETS["capex_GBP"], "£", 100.0),
        ("40-year investor NPV", investor.get("npv_GBP"), REPORT_PHASE1_TARGETS["investor_npv_40y_GBP"], "£", 150.0),
    ]
    return [
        {
            "Check": label,
            ...
            "Unit": unit,
            "Status": "PASS" if actual is not None and abs(float(actual) - target) <= tolerance else "FAIL",
        }
        for label, actual, target, unit, tolerance in rows
    ]
...
def result_summary_row(result: dict[str, Any]) -> dict[str, Any]:
    ...
    cfg = result["input"]
    econ = cfg["economics"]
    source_mix = ", ".join(s.get("type", "?").replace("_", " ") for s in cfg.get("sources", []))
    return {
        "Scenario": result["scenario_name"],
        "Decision": screen.get("status", "NOT ASSESSED"),
        "System": h["system_type"],
        "Heat sources": source_mix,
        "Route (km)": h.get("network_total_length_m", 0) / 1000,
        "Heat density (MWh/m/yr)": h.get("linear_heat_density_MWh_per_m_year"),
        "Heat demand (GWh/yr)": h["annual_heat_demand_MWh"] / 1000,
        "Cooling demand (GWh/yr)": h["annual_cooling_demand_MWh"] / 1000,
        "Peak heat generation (MW)": h.get("peak_heat_to_generate_MW"),
        "CAPEX (£m)": h["capex_total_GBP"] / 1e6,
        "Net CAPEX after grant (£m)": h.get("effective_capex_GBP", h["capex_total_GBP"]) / 1e6,
        ...
        "Worst unmet energy (%)": max(h.get("unmet_heat_fraction", 0), h.get("unmet_cooling_fraction", 0)) * 100,
        "N-1 heat margin (MW)": h.get("n_minus_one_heat_margin_MW"),
        "Service gate": "PASS" if h.get("service_compliant") else "FAIL",
        "Carbon gate": "PASS" if h.get("carbon_compliant") else "FAIL",
        "Investor NPV (£m)": None if inv.get("npv_GBP") is None else inv["npv_GBP"] / 1e6,
        "Investor IRR (%)": None if inv.get("irr") is None else inv["irr"] * 100,
        "Hurdle rate (%)": screen.get("hurdle_rate", econ["discount_rate"]) * 100,
        "Heat tariff basis": inv.get("heat_tariff_mode"),
        "Equivalent year-1 heat tariff (p/kWh)": inv.get("equivalent_year1_heat_tariff_p_per_kWh"),
        "Break-even heat tariff (p/kWh)": inv.get("required_heat_tariff_p_per_kWh_for_zero_NPV"),
        "NPV vs counterfactual (£m)": None if f.get("npv_vs_counterfactual_GBP") is None else f["npv_vs_counterfactual_GBP"] / 1e6,
        "Investor discounted payback (yrs)": inv.get("discounted_payback_years"),
    }
...
def _source_editor(prefix: str, source: dict[str, Any], allowed: dict[str, dict], cooling: bool = False) -> dict[str, Any]:
    ...
    if source_type in {"ashp", "booster_heat_pump", "air_cooled_chiller"}:
        ...
        preset_defaults = allowed[source_type].get(preset, {})
        default_n_units = int(source.get("n_units", preset_defaults.get("n_units", 1)))
        n_units = int(cols[4].number_input("Units", min_value=1, value=default_n_units, step=1,
                                            key=f"{prefix}_units"))
    else:
        cols[4].caption("Single total-capacity asset")
        n_units = None
    edited = copy.deepcopy(source) if source_type == source.get("type") else {}
    edited.update({"type": source_type, "preset": preset, "name": name, "capacity_MW": float(capacity)})
    if n_units is not None:
        edited["n_units"] = n_units
    else:
        edited.pop("n_units", None)
    ...
    with st.expander("Technology-specific settings", expanded=False):
        ...
        if source_type == "data_centre":
            ...
            edited["supply_temp_C"] = d1.number_input(
                "Recovered heat temperature (°C)", min_value=15.0, max_value=60.0,
                value=number(source.get("supply_temp_C"), 30.0), step=1.0,
                key=f"{prefix}_dc_temp",
                help="Air-cooled/chiller systems are commonly around 20-35°C; liquid-cooled systems may be higher.",
            )
            edited["availability_factor"] = d2.number_input(
                "Contracted availability", min_value=0.01, max_value=1.0,
                value=number(source.get("availability_factor"), 0.95), step=0.01,
                key=f"{prefix}_dc_availability",
            )
            edited["waste_heat_cost_GBP_per_MWh"] = d3.number_input(
                "Waste-heat fee (£/MWh)", min_value=0.0,
                value=number(source.get("waste_heat_cost_GBP_per_MWh"), 5.0), step=1.0,
                key=f"{prefix}_dc_heat_fee",
            )
            edited["capex_GBP_per_MW"] = st.number_input(
                "Data-centre recovery CAPEX (£/MW recovered heat)", min_value=0.0,
                value=number(source.get("capex_GBP_per_MW"), 462_000.0), step=25_000.0,
                key=f"{prefix}_dc_capex",
                help="Covers the recovery interface, not the booster heat pump or the data centre itself.",
            )
...
def edit_scenario() -> dict[str, Any]:
    ...
    with st.expander("Commercial assumptions (tariffs, CAPEX and OPEX)", expanded=False):
        ...
        tariff_mode = st.selectbox(
            "Heat customer-bill basis",
            ["counterfactual_bill_parity", "manual"],
            index=0 if tariffs.get("heat_tariff_mode", "counterfactual_bill_parity") == "counterfactual_bill_parity" else 1,
            format_func={
                "counterfactual_bill_parity": "Match each customer's modelled individual-gas bill",
                "manual": "Enter a separate district-heat tariff",
            }.get,
            help="Gas-bill parity links heat revenue to the same gas counterfactual used for the customer comparison, including standing charges.",
        )
        tariffs["heat_tariff_mode"] = tariff_mode
        cooling_tariff_mode = st.selectbox(
            "Cooling customer-bill basis",
            ["counterfactual_bill_parity", "manual"],
            index=0 if tariffs.get("cooling_tariff_mode", "counterfactual_bill_parity") == "counterfactual_bill_parity" else 1,
            format_func={
                "counterfactual_bill_parity": "Match each customer's modelled individual-AC running cost",
                "manual": "Enter a separate district-cooling tariff",
            }.get,
            help="Bill parity prevents district cooling from being made viable by charging customers more than their modelled individual AC alternative.",
        )
        tariffs["cooling_tariff_mode"] = cooling_tariff_mode
        ...
        tariffs["heat_unit_rate_p_per_kWh"] = t1.number_input(
            ...
            disabled=tariff_mode == "counterfactual_bill_parity",
            help="Used only in manual mode. Gas-bill parity derives an equivalent heat price from the individual-gas bill.",
        )
        tariffs["cooling_unit_rate_p_per_kWh"] = t2.number_input(
            ...
            disabled=cooling_tariff_mode == "counterfactual_bill_parity",
        ...
        tariffs["standing_charge_GBP_per_connection_year"] = t3.number_input(
            ...
            disabled=tariff_mode == "counterfactual_bill_parity",
            help="Used only in manual mode; gas-bill parity includes the counterfactual gas standing charge.",
        )
    ...
    with st.expander("Internal screening criteria", expanded=False):
        screening = scenario.setdefault("screening", {})
        s1, s2, s3 = st.columns(3)
        screening["maximum_unmet_energy_fraction"] = s1.number_input(
            "Maximum annual unmet energy (%)", min_value=0.0, max_value=100.0,
            value=number(screening.get("maximum_unmet_energy_fraction"), 0.001) * 100.0,
            step=0.01, format="%.3f",
        ) / 100.0
        screening["maximum_carbon_gCO2e_per_kWh"] = s2.number_input(
            "Maximum carbon (gCO₂e/kWh)", min_value=0.0,
            value=number(screening.get("maximum_carbon_gCO2e_per_kWh"), 100.0), step=5.0,
        )
        screening["investor_hurdle_rate"] = s3.number_input(
            "Investor hurdle rate (%)", min_value=0.0, max_value=100.0,
            value=number(screening.get("investor_hurdle_rate"), econ["discount_rate"]) * 100.0,
            step=0.5,
        ) / 100.0
        s4, s5 = st.columns(2)
        screening["minimum_investor_npv_GBP"] = s4.number_input(
            "Minimum investor NPV (£m)", value=number(screening.get("minimum_investor_npv_GBP"), 0.0) / 1e6,
            step=0.25,
        ) * 1e6
        screening["require_n_minus_one"] = s5.toggle(
            "Require N-1 peak capacity", value=bool(screening.get("require_n_minus_one", False)),
            help="Uses a peak-hour capacity screen. A detailed resilience study is still required later.",
        )
        use_tariff_ceiling = st.toggle(
            "Apply a maximum break-even heat tariff", value=screening.get("maximum_required_heat_tariff_p_per_kWh") is not None,
        )
        screening["maximum_required_heat_tariff_p_per_kWh"] = (
            st.number_input(
                "Maximum break-even heat tariff (p/kWh)", min_value=0.0,
                value=number(screening.get("maximum_required_heat_tariff_p_per_kWh"), 12.0), step=0.25,
            ) if use_tariff_ceiling else None
        )

    st.subheader("2. Buildings and demand")
    st.caption(
        "For each building, use either floor area/units for an archetype estimate, "
        "or enter measured annual energy. Annual overrides take priority for that "
        "service. If no floor area or units are supplied, omitted services are zero."
    )
    original_buildings = copy.deepcopy(scenario["demand"]["buildings"])
    buildings = pd.DataFrame(original_buildings)
    building_columns = [
        "name", "type", "floor_area_m2", "units", "annual_heat_kWh",
        "annual_cool_kWh", "annual_dhw_kWh", "peak_total_heat_kW",
        "connections", "connection_year", "connection_probability",
    ]
    for col in building_columns:
        if col not in buildings:
            buildings[col] = None
    buildings = buildings[building_columns]
    edited_buildings = st.data_editor(
        ...
        column_config={
            ...
            "annual_heat_kWh": st.column_config.NumberColumn(
                "Annual space heat override (kWh)", min_value=0.0, step=1000.0,
                help="May be used without floor area. If your meter total includes DHW, enter the full value here and set annual DHW to 0.",
            ),
            "annual_cool_kWh": st.column_config.NumberColumn(
                "Annual cooling override (kWh)", min_value=0.0, step=1000.0,
                help="Optional measured cooling. With no floor area, a blank value means zero cooling.",
            ),
            "annual_dhw_kWh": st.column_config.NumberColumn(
                "Annual DHW override (kWh)", min_value=0.0, step=1000.0,
                help="Optional separate domestic-hot-water demand. Enter 0 when it is already included in the annual heat figure.",
            ),
            "peak_total_heat_kW": st.column_config.NumberColumn(
                "Peak total heat override (kW)", min_value=0.0, step=10.0,
                help="Optional. If blank, the model estimates the peak from the annual heat and archetype hourly shape.",
            ),
    ...
    original_by_name = {b.get("name"): b for b in original_buildings}
    edited_rows = []
    for row in edited_buildings.to_dict("records"):
        ...
        preserved = copy.deepcopy(original_by_name.get(clean.get("name"), {}))
        preserved.update(clean)
        edited_rows.append(preserved)
    ...
    report_validation_case = _is_ealing_validation_name(scenario.get("name"))
    if report_validation_case:
        auto_col1.caption(
            "Ealing validation uses the report's fixed 2.8 MW ASHP, 3.6 MW boiler "
            "and aggregate peak. Rename the scenario before testing a different design."
        )
    if auto_col2.button(
        "Auto-size from demand", type="secondary",
        help="Let the model recommend technology capacities based on the demand profile above",
        disabled=report_validation_case,
    ):
        import copy as _cp
        ...
        try:
            ...
            # Preserve aggregate peak/shape calibration when a scenario has it.
            # Passing only demand_nodes previously ignored the Ealing report's
            # diversified network peak and sized from a different profile.
            _demand_cfg = _cp.deepcopy(_auto_sc["demand"])
            _demand_cfg["demand_nodes"] = _demand_cfg["buildings"]
            _d = _synth(_w, _demand_cfg)
    ...
    st.subheader("6. Thermal storage and peak coverage")
    store = scenario.setdefault("thermal_storage", {"enabled": False})
    store["enabled"] = st.toggle(
        "Include thermal storage", value=bool(store.get("enabled", False)),
        help="Storage is dispatched hourly and can either displace boiler heat or be reserved for plant-capacity shortfalls.",
    )
    if store["enabled"]:
        s1, s2, s3, s4 = st.columns(4)
        store["volume_litres"] = s1.number_input(
            "Store volume (litres)", min_value=1.0,
            value=number(store.get("volume_litres"), 50_000.0), step=5_000.0,
        )
        store.pop("capacity_MWh", None)
        store["delta_T_K"] = s2.number_input(
            "Usable store ΔT (K)", min_value=1.0,
            value=number(store.get("delta_T_K"), 40.0), step=1.0,
        )
        store["max_charge_MW"] = s3.number_input(
            "Maximum charge (MW)", min_value=0.01,
            value=number(store.get("max_charge_MW"), 2.0), step=0.1,
        )
        store["max_discharge_MW"] = s4.number_input(
            "Maximum discharge (MW)", min_value=0.01,
            value=number(store.get("max_discharge_MW"), 2.0), step=0.1,
        )
        strategy_options = ["peak_reserve", "displace_boiler"]
        store["dispatch_strategy"] = st.selectbox(
            "Storage dispatch purpose", strategy_options,
            index=strategy_options.index(store.get("dispatch_strategy", "peak_reserve")),
            format_func={
                "peak_reserve": "Reserve for plant-capacity shortfalls",
                "displace_boiler": "Displace boiler heat whenever charged",
            }.get,
        )
        store["capex_GBP"] = st.number_input(
            "Storage installed CAPEX (£)", min_value=0.0,
            value=number(store.get("capex_GBP"), 0.0), step=10_000.0,
        )
...
def show_result(result: dict[str, Any]) -> None:
    ...
    if _is_ealing_validation_name(result.get("scenario_name")):
        validation_rows = _ealing_validation_rows(result)
        validation_ok = all(row["Status"] == "PASS" for row in validation_rows)
        if validation_ok:
            st.success("Ealing report calibration check passed: this run reproduces the fixed Phase 1 validation case.")
        else:
            st.error(
                "This run does not reproduce the Ealing report validation case. "
                "Its inputs are modified or stale; do not interpret it as an Ealing result. "
                "Reload the Ealing template before continuing."
            )
        with st.expander("Ealing report calibration checks", expanded=not validation_ok):
    ...
    with view_tab2:
        ...
        if inv:
            ...
            eq_tariff = inv.get("equivalent_year1_heat_tariff_p_per_kWh")
            r3.metric("Equivalent year-1 heat bill", "—" if eq_tariff is None else f"{eq_tariff:.2f} p/kWh")
    ...
    with st.expander("Energy service detail", expanded=False):
        ...
        ec1.metric("Peak heat to generate", f"{h.get('peak_heat_to_generate_MW', h['peak_heat_MW']):.2f} MW")
        ec1.metric("Network heat loss", f"{h['annual_network_heat_loss_MWh']:.0f} MWh/year")
        if h.get("linear_heat_density_MWh_per_m_year") is not None:
            ec1.metric("Linear heat density", f"{h['linear_heat_density_MWh_per_m_year']:.2f} MWh/m/year")
        ec1.metric("Network loss share", f"{h.get('network_heat_loss_fraction', 0)*100:.1f}%")
        ec2.metric("Peak available heat capacity", f"{h.get('peak_available_heat_capacity_MW', 0):.2f} MW")
        ec2.metric("N-1 firm heat capacity", f"{h.get('n_minus_one_heat_firm_capacity_MW', 0):.2f} MW")
        ec2.metric("N-1 heat margin", f"{h.get('n_minus_one_heat_margin_MW', 0):+.2f} MW")
        if h.get("annual_total_parasitic_electricity_MWh", 0):
            ec1.metric("Total parasitic electricity", f"{h['annual_total_parasitic_electricity_MWh']:.0f} MWh/year")
        if h.get("thermal_storage_capacity_MWh", 0):
            ec2.metric("Thermal storage", f"{h['thermal_storage_capacity_MWh']:.2f} MWh")
            ec2.metric("Storage discharge", f"{h.get('thermal_storage_discharge_MWh', 0):.1f} MWh/year")
    ...
    with st.expander("CAPEX and annual OPEX breakdown", expanded=False):
        ...
        opex_audit = f.get("opex_reconciliation", {})
        if opex_audit:
            st.caption(opex_audit.get("basis", ""))
            st.dataframe(pd.DataFrame([
                {"OPEX line": key.replace("_GBP", "").replace("_", " ").title(), "£/year": value}
                for key, value in opex_audit.items()
    ...
    # -- One auditable screening verdict --
    st.subheader("Internal screening decision")
    screen = result.get("screening", {})
    status = screen.get("status", "NOT ASSESSED")
    if status == "PASS":
        st.success(f"{status}: {screen.get('summary', '')}")
    elif status == "CONDITIONAL PASS":
        st.warning(f"{status}: {screen.get('summary', '')}")
    else:
        st.error(f"{status}: {screen.get('summary', '')}")
    gate_rows = []
    for gate in screen.get("gates", []):
        passed = gate.get("passed")
        gate_rows.append({
            "Gate": gate["name"],
            "Required": "Yes" if gate.get("required") else "No (information)",
    ...
    if screen.get("evidence_flags"):
        with st.expander("Evidence actions before external use", expanded=True):
            for flag in screen["evidence_flags"]:
                st.write(f"• {flag}")
    st.caption(screen.get("scope", ""))
...
def show_comparison() -> None:
    ...
    if not results:
        st.info("Run a scenario and add it here, or run a comparison set from the sidebar.")
        return
    rows = [result_summary_row(r) for r in results]
    df = pd.DataFrame(rows)
    st.dataframe(df, use_container_width=True, hide_index=True)

    chart1, chart2, chart3 = st.tabs(["NPV and carbon", "Lifetime cash position", "Choose a metric"])
    with chart1:
        plot_df = df.dropna(subset=["Investor NPV (£m)", "Carbon (gCO₂e/kWh)"]).copy()
        if not plot_df.empty:
            scatter = (
                alt.Chart(plot_df)
                .mark_circle(opacity=0.85)
                .encode(
                    x=alt.X("Carbon (gCO₂e/kWh):Q", title="Operational carbon (gCO₂e/kWh)"),
                    y=alt.Y("Investor NPV (£m):Q", title="Investor NPV (£m)"),
                    size=alt.Size("CAPEX (£m):Q", title="Gross CAPEX (£m)"),
                    color=alt.Color("Decision:N"),
                    tooltip=["Scenario:N", "Decision:N", alt.Tooltip("Investor NPV (£m):Q", format=".2f"),
                             alt.Tooltip("Carbon (gCO₂e/kWh):Q", format=".1f"),
                             alt.Tooltip("CAPEX (£m):Q", format=".2f")],
                )
                .properties(height=360)
            )
            st.altair_chart(scatter, use_container_width=True)
            st.caption("Best screening region is upper-left: positive investor value with low operational carbon.")
    with chart2:
        lifetime_rows = []
        for result in results:
            inv = result.get("financial", {}).get("investor", {})
            for year, value in zip(inv.get("cashflow_years", []), inv.get("cumulative_discounted_GBP", [])):
                lifetime_rows.append({"Scenario": result["scenario_name"], "Year": year, "Cumulative discounted (£m)": value / 1e6})
        if lifetime_rows:
            lifetime = (
                alt.Chart(pd.DataFrame(lifetime_rows))
                .mark_line()
                .encode(
                    x=alt.X("Year:Q", axis=alt.Axis(tickMinStep=1)),
                    y=alt.Y("Cumulative discounted (£m):Q"),
                    color="Scenario:N",
                    tooltip=["Scenario:N", "Year:Q", alt.Tooltip("Cumulative discounted (£m):Q", format=".2f")],
                )
                .properties(height=360)
            )
            st.altair_chart(lifetime, use_container_width=True)
    with chart3:
        numeric_cols = list(df.select_dtypes(include="number").columns)
        default_metric = "Investor NPV (£m)" if "Investor NPV (£m)" in numeric_cols else numeric_cols[0]
        metric = st.selectbox("Graph metric", numeric_cols, index=numeric_cols.index(default_metric))
        st.bar_chart(df.set_index("Scenario")[[metric]])
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        st.divider()
        comparison_sets = {
            "Worked technology options": WORKED_SCENARIOS,
            "Representative technology mixes": [
                frontier_scenario(mix, 2.0, 1.0) for mix in MIX_NAMES
            ],
            "Heat-network feasibility routes": FEASIBILITY_SCENARIOS,
            "Data-centre waste-heat cases": DATA_CENTRE_SCENARIOS,
        }
        comparison_set_name = st.selectbox("Comparison set", list(comparison_sets))
        if st.button("Run selected comparison set", use_container_width=True):
            with st.spinner("Running comparison scenarios..."):
                st.session_state.comparison_results = [
                    run_scenario(copy.deepcopy(s)) for s in comparison_sets[comparison_set_name]
                ]
            st.success("Comparison set completed.")
